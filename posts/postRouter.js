...
router.get('/', (req, res) => {
  posts
    ...
    .catch(err => res.status(404).json({ errorMessage: "could not find all posts", err }));
});

router.post('/', validatePostId(), validatePost(), (req, res) => {
  posts
    .insert(req.text)
    .update(req.params.id, req.text)
    .then(data => res.json(data))
    .catch(err => res.status(404).json({ message: "could not add post", err }));
});
router.get('/:id', validatePostId(), (req, res) => {
  posts.getById(req.params.id)
    .then(post => res.json(post))
    .catch(err => res.status(404).json({errorMmessage: 'could not find posts with this ID', err}))
  });
